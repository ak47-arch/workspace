...
|----------------------|-----------|--------------------|---------------------|---------------|--------------------|
| Implement the implementer / reviewer / factory ORCHESTRATION | [Software Factory](/openwiki/projects/software-factory.md) | `bin/implementer-run.sh`, `bin/review-run.sh`, `bin/factory-run.sh`, `bin/merge-pr.sh` | `--pick`, `--revise`, `--continue`, `factory:needs-review` label | `bin/test-implementer-driver.sh`, `bin/test-review-driver.sh`, `bin/test-factory-run.sh`, `bin/test-merge-pr.sh` | `bin/test-factory-run.sh` |
| Review a raised PR | [Software Factory](/openwiki/projects/software-factory.md) | `bin/review-run.sh` | `code-reviewer`, APPROVE/REQUEST_CHANGES verdict | `bin/test-review-driver.sh` | `bin/review-run.sh --dry-run <pr>` |
...
Recent additions (software-factory): implemented the **implementer agent** (`bin/implementer-run.sh`), the **code-reviewer agent** (`bin/review-run.sh`), a **code-review sub-agent** refactor, **ponytail-skills integration**, an **implementer revision mode** (`--revise <pr>`), the **factory-run orchestrator** (`bin/factory-run.sh`), and the **operator merge tool** (`bin/merge-pr.sh`). `extension-inline-agent` reached `complete` via a raised PR.

Selected project tasks:
- **software-factory**: automated monitoring thinking, opensource/cognee evaluation, improve shared context infrastructure, add task-selection abstraction layer, extend the prod review agent, add headless herdr host, no-prompt-before-delete infrastructure, build the factory into its own product with evals, host apps in test/main/prod environments, save compressed sessions (all pending)
