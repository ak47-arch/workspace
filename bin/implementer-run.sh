...
resolve_prd() {
  ...
  if ! grep -qE '^\*\*Status\*\*: *\[?Final' "$prd"; then
...
transition() {
  ...
  # One-PR-per-task: target the task branch inside the worktree clone (which is the
  # branch that gets pushed for the PR) so lifecycle bookkeeping rides up with the
  # implementation instead of accumulating on a protected default branch.
  if [ -n "$WORKTREE_BRANCH" ] && [ -n "$WORKTREE" ] && [ -d "$WORKTREE/.git" ]; then
    "$WORKSPACE/bin/transition-task.sh" "$PRD_SLUG" --to "$to" $session_flag --branch "$WORKTREE_BRANCH" --workspace "$WORKTREE" "$@"
  else
