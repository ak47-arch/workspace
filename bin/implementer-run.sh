...
# - $MODE_FLAG=--pick: oldest `**Status**: Final` in the stable docs/prd/
#   candidate set whose task is `**Status**: prd-ready` (the lifecycle's
#   queued-entry state — decision 09). PRDs live on a stable home (never a
#   moving queue/archive); resolve_prd globs docs/prd/*.md and reads each
#   candidate's Status (a [+final] typed link to the manifest also matches) +
#   the task's prd-ready gate. Oldest = smallest date prefix (ordering_key
#   preserved by the routing manifest). in-progress (concurrent owner) and
#   in-review (merged/blocked/verifying lineages) tasks are NOT pickable; a
#   stale Final PRD for an in-flight or merged task must never be re-implemented.
...
resolve_prd() {
  ...
  if [ -n "$MODE_FLAG" ]; then
    ...
    for candidate in "$WORKSPACE"/docs/prd/*.md; do
      ...
      grep -qE '^\*\*Status\*\*: *\[?Final' "$candidate" || continue
